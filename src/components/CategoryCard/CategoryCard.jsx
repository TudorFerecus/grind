...
const CategoryCard = ({ category }) => {
  ...
  return (
    <Link to={`/category/${category.slug}`} className="relative group overflow-hidden rounded-2xl shadow-md border border-base-200 aspect-[4/3] block bg-base-300">
      <img src={category.imageUrl || category.image} alt={category.name} className="absolute inset-0 w-full h-full object-cover transition-transform duration-700 ease-out group-hover:scale-110" loading="lazy" />
      ...
      <div className="absolute inset-0 p-6 flex flex-col justify-end">
        ...
        <div className="transform transition-transform duration-300 group-hover:-translate-y-2">
          ...
          <p className="text-primary/80 text-sm mb-4 line-clamp-2 opacity-0 group-hover:opacity-100 h-0 group-hover:h-auto transition-all duration-300">
